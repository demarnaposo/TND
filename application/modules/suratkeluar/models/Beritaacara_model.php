...
class beritaacara_model extends CI_Model
{
	public function get_data($tahun,$opd_id,$jabatan_id)
	{
		// $query = $this->db->query("
		// 	SELECT * FROM draft
		// 	LEFT JOIN surat_beritaacara
		// 	ON draft.surat_id = surat_beritaacara.id
		// 	LEFT JOIN opd
		// 	ON opd.opd_id = surat_beritaacara.opd_id
		// 	LEFT JOIN aparatur
		// 	ON aparatur.jabatan_id = draft.penandatangan_id
		// 	WHERE surat_beritaacara.opd_id = '$opd_id'
		// 	AND LEFT(surat_beritaacara.tanggal, 4) = '$tahun'
		// 	AND draft.dibuat_id = '$jabatan_id'
		// 	ORDER BY surat_beritaacara.tanggal DESC, LENGTH(surat_beritaacara.id) DESC, surat_beritaacara.id DESC
		// ");
		// return $query;
	...
	// {
	// 	if($table == "" || $table == null){
	// 		$query = $this->db->query("SELECT * FROM surat_beritaacara ORDER BY LENGTH(id) ASC, id ASC");
	// 	}else{
