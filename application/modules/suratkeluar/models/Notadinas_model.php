...
class Notadinas_model extends CI_Model
{
	...
	public function get_data($tahun,$opd_id,$jabatan_id)
	{
		// $query = $this->db->query("
		// 	SELECT * FROM draft
		// 	LEFT JOIN surat_notadinas
		// 	ON draft.surat_id = surat_notadinas.id
		// 	LEFT JOIN opd
		// 	ON opd.opd_id = surat_notadinas.opd_id
		// 	LEFT JOIN aparatur
		// 	ON aparatur.jabatan_id = draft.penandatangan_id
		// 	WHERE surat_notadinas.opd_id = '$opd_id'
		// 	AND LEFT(surat_notadinas.tanggal, 4) = '$tahun'
		// 	AND draft.dibuat_id = '$jabatan_id'
		// 	ORDER BY surat_notadinas.tanggal DESC, LENGTH(surat_notadinas.id) DESC, surat_notadinas.id DESC
		// ");
		// return $query;
