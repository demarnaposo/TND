...
class Biasa extends CI_Controller {
	...
	public function insert()
	{
		...
		//Untuk menambahkan surat
			...
			if (empty($file)) {
				...
				//form validation surat biasa [@dam|E-Gov 10.04.2022]
				...
				if ($insert) {
		            ...
		            internal_eksternal_tembusan('biasa',$surat_id,$tembusan_id,$tembusaneks_id);
				...
				$ekstensi = end($ambext);
				$date = date('his');
			    $nama_baru = "Lampiran-Surat-Biasa-No-".$surat_id."-".$date;
	...
	public function update()
	{
		...
		//Untuk mengedit surat
		    ...
		    if (!empty($tembusan_id) OR !empty($tembusaneks_id)) {
		    	internal_eksternal_tembusan('biasa',$id,$tembusan_id,$tembusaneks_id);
	    	}
			...
			if (empty($file)) {
				...
				$ekstensi = end($ambext);
				$date = date('his');
			    $nama_baru = "Lampiran-Surat-Biasa-No-".$id."-".$date;
				$nama_file = $nama_baru.".".$ekstensi;	
				$config['upload_path'] = './assets/lampiransurat/biasa/';
				$config['allowed_types'] = 'pdf';
				$config['file_name'] = $nama_file;
	...
	public function delete()
	{
		...
		if ($delete) {
			...
			$this->biasa_model->delete_data('tembusan_surat', $whereDis);
	...
	public function delete_tembusan()
	{
		...
		$delete = $this->biasa_model->delete_data('tembusan_surat', $where);
		if ($delete) {
