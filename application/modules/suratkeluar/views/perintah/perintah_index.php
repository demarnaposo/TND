...
<ul class="breadcrumb">
    ...
    <li class="active">Surat Perintah Khusus</li>
</ul>
<!-- END BREADCRUMB -->

<!-- PAGE TITLE -->
<div class="page-title">                    
    <h2><span class="fa fa-envelope"></span> Surat Perintah Khusus</h2>
</div>
<!-- END PAGE TITLE -->                
        ...
        <div class="col-md-12">
            ...
            <div class="panel panel-default">
                <div class="panel-body">
                    ...
                    <a href="<?php echo site_url('suratkeluar/perintah/add') ?>" class="btn btn-primary"><i class="fa fa-plus"></i>Buat Surat</a> <br><br>
                    <?php } ?>
                    ...
                    <table class="table datatable table-bordered">
                        <thead>
                            <tr>
                                ...
                                <th width="12%">PEMBERI PERINTAH</th>
				<th width="12%">YANG DIBERI PERINTAH</th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                <?php
                                    $kode=$this->db->query("SELECT kode_surat.kode, kode_surat.tentang FROM surat_perintah LEFT JOIN kode_surat ON kode_surat.kodesurat_id=surat_perintah.kodesurat_id WHERE surat_perintah.kodesurat_id='$h->kodesurat_id'")->row_array();
                                ...
                                <td>
                                    <?php
                                    $kop_id=$this->db->query("SELECT kop_surat.nama FROM surat_perintah LEFT JOIN kop_surat ON kop_surat.kop_id=surat_perintah.kop_id WHERE surat_perintah.kop_id='$h->kop_id'")->row_array();
                                    echo $kop_id['nama'];
                                    ?>
                                </td>
                                <td>
                                ...
                                <?php
                                if($h->kop_id==1){
                                    $pemberiperintah=$this->db->query("SELECT nama FROM aparatur WHERE jabatan_id = 1")->row_array();
                                ?>
                                <td><?= $pemberiperintah['nama'] ?></td>
                                <?php 
                                    }else{

                                $id=$h->surat_id;
                                $pemberiperintah=$this->db->query("SELECT b.nama FROM surat_perintah a LEFT JOIN penandatangan b ON b.surat_id=a.id LEFT JOIN draft c ON c.surat_id=a.id WHERE a.id='$id'")->result();
                                foreach($pemberiperintah as $key){
                                ?>
								...
								<td><?php
									...
									$no = 1;
									    foreach ($datapegawai->result() as $key => $p) {
    										if($datapegawai->num_rows()=='1'){
    										    echo $p->nama; echo "<br>";
    										}else{
        										echo $no. ". "; echo $p->nama; echo "<br>";
        										$no++;  
    										}
									    } 
									}
                                ...
                                <td style="font-weight: bold;">
                                    ...
                                    <?php 
                                        $query=$this->db->query("SELECT * FROM penandatangan WHERE surat_id='$h->id'")->row_array(); //Update @Mpik Egpv 22/06/2022
                                        $qverifikasi = $this->db->query("
                                            SELECT * FROM verifikasi 
                                            JOIN surat_perintah ON verifikasi.surat_id = surat_perintah.id
                                            WHERE verifikasi.surat_id = '$h->id'
                                        ")->num_rows();
                                        if (empty($qverifikasi)) {
                                            echo "<p style='color:red; text-align:center;'> KONSEP SURAT </p>";
                                        }elseif($query['status'] == 'Belum Ditandatangani'){ //Update @Mpik Egpv 22/06/2022
                                            echo "<p style='color:red; text-align:center;'> SURAT SEDANG PROSES TTE </p>"; //Update @Mpik Egpv 22/06/2022
                                        }else{
                                    ?>
					...
					<!-- Kondisi jika surat belum ditandatangan tidak bisa preview surat -->
                                        ...
                                        <?php }else{?>
                                            <a href="<?php echo site_url('export/perintah/'.$h->id) ?>" title="Lihat Surat" target="_blank"><span class="fa-stack"><i class="fa fa-eye fa-stack-2x"></i></span></a>                                    
                                        <?php }?>
                                        <!-- Kondisi jika surat belum ditandatangan tidak bisa preview surat --> 
                                    ...
                                    ?>
                                        <a href="javascript:void(0)" data-toggle="modal" data-target="#modalLihat<?php echo $h->id ?>" title="Lihat Surat"><span class="fa-stack"><i class="fa fa-file-pdf-o fa-stack-2x"></i></span></a>
                                        ...
                                        <div class="modal fade" id="modalLihat<?php echo $h->id; ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                          <div class="modal-dialog" role="document">
                                            <div class="modal-content">
                                              ...
                                              <div class="modal-body">
                                                <a href="<?php echo site_url('export/perintah/'.$h->id) ?>" title="Lihat Surat" target="_blank" style="text-decoration: none;" class="btn btn-danger">Lihat Surat</a>
                                                <a href="<?php echo base_url('assets/lampiransurat/perintah/'.$h->lampiran_lain) ?>" title="Lihat Lampiran" target="_blank" style="text-decoration: none;" class="btn btn-warning">Lihat Lampiran</a>
                                              </div>
                                              <div class="modal-footer">
                                                <button type="button" class="btn btn-secondary" data-dismiss="modal">Tutup</button>
                                              </div>
...
<!--============ START [UPDATE] Fikri Egov ================ [UPDATE] Fikri Egov ============================ [UPDATE] Fikri Egov ========================= [UPDATE] Fikri Egov ================================================-->
                                    ...
                                    ?>
                                    | <a href="<?php echo site_url('suratkeluar/perintah/edit/'.$h->id) ?>" data-toggle="tooltip" data-placement="top" title="Ubah Surat"><span class="fa-stack"><i class="fa fa-pencil fa-stack-2x"></i></span></a> 
                                    
                                    <?php if ($h->dibuat_id == $this->session->userdata('jabatan_id')) {  ?>
                                    | <a href="<?php echo site_url('suratkeluar/perintah/delete/'.$h->id) ?>" data-toggle="tooltip" data-placement="top" title="Hapus Surat" onclick="return confirm('Apakah anda yakin akan menghapus?')"><span class="fa-stack"><i class="fa fa-trash-o fa-stack-2x"></i></span></a>
