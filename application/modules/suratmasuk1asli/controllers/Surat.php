...
class Surat extends CI_Controller {
	...
	public function index()
	{
		...
		// config pagination
		$config['base_url']=base_url("suratmasuk1/surat/index/");
		...
		if (empty($surat_id)) {
			...
			if (empty($file)) {
				...
				if(!$this->upload->do_upload('lampiran')){
					...
					redirect(site_url('suratmasuk1/surat/add'));
				}else{
					$data = array(
					...
					if($cekdoubleinput == 1){
						...
						redirect(site_url('suratmasuk1/surat'));
					}else{
...
public function disposisi()
{
	if (isset($_POST['disposisi'])) {
		...
		if($cekdisposisi==1){
			...
			redirect(site_url('suratmasuk1/surat/detaildata/'.$suratmasukid));
		}else{
