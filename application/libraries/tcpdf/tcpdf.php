...
class TCPDF {
	...
	{ 
	   ...
	   //$logoX = 10; // 
	   //$logoFileName = "assets/img/bsre.png";
	   //$logoWidth = 21; // 15mm
	   //$logoY = 316;
	   ...
	   //$this->Cell(10,10, $logo, 0, 0, 'C');
        ...
        // $this->write2DBarcode($_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'], 'QRCODE,H', 12, 305, 15, 15, $style, 'N');
	   
		$uri_path = parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH); // Update @Mpik Egov 2 Agust 2022
        $uri_segments = explode('/', $uri_path); // Update @Mpik Egov 2 Agust 2022
        
        $ci =& get_instance(); // Update @Mpik Egov 2 Agust 2022
        // $cekquery=$ci->db->query("SELECT status FROM penandatangan WHERE surat_id='$uri_segments[3]'")->row_array(); // Update @Mpik Egov 2 Agust 2022
        $cekquery=$ci->db->query("SELECT a.status,a.jabatan_id, c.opd_id,d.kopId FROM penandatangan a 
        LEFT JOIN jabatan b ON b.jabatan_id=a.jabatan_id
        LEFT JOIN opd c ON c.opd_id=b.opd_id
        LEFT JOIN draft d ON d.surat_id=a.surat_id
        WHERE a.surat_id='$uri_segments[3]'")->row_array(); // Update @Mpik Egov 2 Agust 2022
        
        // $this->write2DBarcode($_SERVER['HTTP_HOST'] . '/'.$cekquery['status'], 'QRCODE,H', 12, 305, 15, 15, $style, 'N');
		$kopId=$ci->db->query("SELECT a.kopId,c.opd_id FROM draft a LEFT JOIN jabatan b ON b.jabatan_id=a.dibuat_id LEFT JOIN opd c ON c.opd_id=b.opd_id WHERE a.surat_id='$uri_segments[3]'")->row_array();
		if ($cekquery['status'] == 'Belum Ditandatangani' || $cekquery['status'] == 'Sudah Ditandatangani') { // Update @Mpik Egov 2 Agust 2022
				if($cekquery['opd_id'] == 22 && $cekquery['kopId'] == 2 ){
					$this->Image(K_PATH_IMAGES.'logo-iso.png',12, 305, 18, 15); // Update @Mpik Egov 6 Januari 2023
					$this->write2DBarcode($_SERVER['HTTP_HOST'] . '/uploads/SIGNED/'.$uri_segments[3].'.pdf', 'QRCODE,H', 32, 305, 15, 15, $style, 'N'); // Update @Mpik Egov 2 Agust 2022
				}else{
					$this->write2DBarcode($_SERVER['HTTP_HOST'] . '/uploads/SIGNED/'.$uri_segments[3].'.pdf', 'QRCODE,H', 12, 305, 15, 15, $style, 'N'); // Update @Mpik Egov 2 Agust 2022
				}
			}else { // Update @Mpik Egov 2 Agust 2022
					if($kopId['opd_id'] == 22 && $kopId['kopId'] == '2' ){
						$this->Image(K_PATH_IMAGES.'logo-iso.png',12, 305, 18, 15); // Update @Mpik Egov 6 Januari 2023
						$this->write2DBarcode($_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'], 'QRCODE,H', 32, 305, 15, 15, $style); // Update @Mpik Egov 2 Agust 2022
					}elseif($ci->session->userdata('opd_id') == 22 && $kopId['kopId'] == '0' ){
						$this->write2DBarcode($_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'], 'QRCODE,H', 12, 305, 15, 15, $style); // Update @Mpik Egov 2 Agust 2022
					}else{
						$this->write2DBarcode($_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'], 'QRCODE,H', 12, 305, 15, 15, $style); // Update @Mpik Egov 2 Agust 2022
					}
	   ...
	   $this->SetY(-21);
		...
		$ketCert ='Dokumen ini telah ditandatangani secara elektronik menggunakan sertifikat elektronik yang diterbitkan BSrE. Untuk memastikan keasliannya, silahkan scan QRCode dan pastikan diarahkan ke alamat https://tnd.kotabogor.go.id';	
			if($kopId['opd_id'] == 22 && $kopId['kopId'] == 2 ){
			  $this->MultiCell(148, 0, $ketCert, 0, 'L', 0, 0, 50, '', true, 50, false, true, 15, 'M');  
			}else{
				$this->MultiCell(165, 0, $ketCert, 0, 'L', 0, 0, 33, '', true, 0, false, true, 8, 'M');  
			}
