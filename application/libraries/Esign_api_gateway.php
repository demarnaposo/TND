...
class Esign_api_gateway
{
	...
	{
		$this->url = 'https://devesign.kotabogor.go.id/api/sign/pdf';
	...
	protected function actionSuccess($response)
	{
		...
		file_put_contents("./uploads/backup/$newFileName", $response);
		...
		curl_setopt_array($curl, array(
			...
			CURLOPT_POSTFIELDS => array('symetric_key' => 'VRoBggoctlkaV3q24H8UrAbF53ctzkp8', 'surat_id' => $surt, 'dokumen_url[0]' => 'https://tnd.kotabogor.go.id/uploads/SIGNED/' . $newFileName),
...
// 		$newFileName = random_string('alpha', 30) . '.pdf';
	...
	protected function generatePDF()
	{
		...
		file_put_contents("./uploads/backup/$newFileName", $binnaryContent);
	...
	public function execute()
	{
		try {
			...
			// Delete Trash File 
			if (file_exists("./uploads/backup/$pdf")) {
				unlink("./uploads/backup/$pdf");
			}
