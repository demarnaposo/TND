...
class Fpdi extends \TCPDF
{
    ...
    protected function getNextTemplateId()
    {
     ...
     public function setSuratid($suratid){
         return $this->suratid = $suratid;
     }
     //end by Inez
    ...
    public function Footer(){
        ...
        /*
        //$this->write2DBarcode($_SERVER['HTTP_HOST'] .'/tnd2/assets/lampiransurat/suratlainnya/'.$this->filename, 'QRCODE,H', 12, 305, 15, 15, $style);
        $this->write2DBarcode($_SERVER['HTTP_HOST'] .'/uploads/SIGNED/'.$this->suratid.'.pdf', 'QRCODE,H', 12, 305, 15, 15, $style);
        $this->SetY(-21);
        ...
        */
        ...
        if($this->CurOrientation == 'P'){ //Portrait mode
            $this->SetXY($this->original_lMargin - 15, -21);
            $this->write2DBarcode($_SERVER['HTTP_HOST'] .'/uploads/SIGNED/'.$this->suratid.'.pdf', 'QRCODE,H', $this->original_lMargin, $this->h - PDF_MARGIN_BOTTOM, 15, 15, $style);
            $this->MultiCell(165, 0, $ketCert, 0, 'L', 0, 0, 33, '', true, 0, false, true, 8, 'M');
        }else{ //Landscape mode
            $this->SetXY($this->original_lMargin - 10, $this->h - 21);
            $this->write2DBarcode($_SERVER['HTTP_HOST'] .'/uploads/SIGNED/'.$this->suratid.'.pdf', 'QRCODE,H', $this->original_lMargin, $this->h - PDF_MARGIN_BOTTOM, 15, 15, $style);
            $this->MultiCell(0, 0, $ketCert, 0, 'L', 0, 0, 40, '', true, 0, false, true, 8, 'M');
        }
